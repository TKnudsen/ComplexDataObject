...
public class MutualInformationFeatureSelection implements INumericalFeatureVectorProcessor {
	...
	private List<NumericalFeatureVector> labeledFVs = null;

	private List<String> labels = null;

	private int nrFeatures;

	public MutualInformationFeatureSelection(Collection<NumericalFeatureVector> labeledFVs, List<String> labels,
	...
	public MutualInformationFeatureSelection(List<NumericalFeatureVector> labeledFVs, String labelAttributeName,
			int nrOfFeatureToSelect) {
		if (labeledFVs == null || labeledFVs.size() == 0) {
			throw new IllegalArgumentException("List must not be null or of size 0");
		}
		labels = new ArrayList<>();
		for (NumericalFeatureVector fv : labeledFVs) {
	...
	private double[] calculateLabelMI() {
		NumericalFeatureVector fv0 = labeledFVs.get(0);
		...
		for (int i = 0; i < fv0.sizeOfFeatures(); i++) {
			if (fv0.getFeature(i).getFeatureType() == FeatureType.DOUBLE) {
				...
				throw new IllegalArgumentException(
						getClass().getSimpleName() + ": not applicable for non-numerical features");
		}
	...
	public List<String> getSortedFeatureNames() {
		...
		NumericalFeatureVector sample = labeledFVs.get(0);
